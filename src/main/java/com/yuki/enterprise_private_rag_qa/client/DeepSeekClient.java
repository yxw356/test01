...
public class DeepSeekClient {
    ...
    private List<Map<String, String>> buildMessages(String userMessage,
        ...
        sysBuilder.append("本轮参考信息会随当前用户问题一起给出。")
                ...
                .append("当参考信息来自企业知识库文件时，你必须像制度问答助手一样先理解问题意图，")
                .append("把问题拆成若干要点，在参考文件中逐项找依据，再形成结构化回答。")
                .append("如果用户的问题可以拆成多个子问题，请逐项回答；如果问题里有模糊指代，")
                .append("优先结合最近对话和本轮参考信息解析指代。")
                .append("不要只复述单个片段；如果多个文件或片段互相补充，应综合归纳。")
                .append("遇到数字、名单、时间、比例、金额、面积、岗位、部门等精确信息，")
                .append("必须从参考信息中原样提取，不能凭常识补齐。")
                .append("如果参考信息包含“案例参考”，只能作为辅助解释或历史处理口径；")
                .append("当案例与正式制度文件冲突时，必须以正式制度文件为准并提示需要人工确认。")
                .append("如果参考信息存在生效时间、废止时间、版本号或审计状态，回答时必须说明适用边界。")
                .append("严禁输出思考过程、英文分析或中间推理，只输出面向用户的最终简体中文回答。");
    ...
    private String buildCurrentTurnContent(String userMessage, String context, AiProperties.Prompt promptCfg) {
        ...
        if (context != null && !context.isBlank()) {
            builder.append("请严格根据下面的本轮参考信息回答问题。")
                    ...
                    .append("先在内部完成问题拆解和证据匹配，但不要输出思考过程。")
                    .append("最终回答建议采用：直接结论、依据拆解、操作/流程、适用边界、注意事项。")
                    .append("如果用户问的是制度、流程、条件、标准、金额、时间、责任人等，请逐条列出。")
                    .append("如果用户问“为什么、如何、哪些、是否、能不能”，请先给结论，再解释判断路径。")
                    .append("数字、单位、面积、金额必须与参考原文完全一致，禁止改写位数。")
                    .append("当不同片段存在冲突时，优先使用更新版本、生效中、审计通过的文件；并说明存在冲突。")
